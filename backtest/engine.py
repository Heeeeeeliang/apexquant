...
class BacktestEngine:
    ...
    def __init__(
        ...
        self._equity_events: dict[datetime, float] = {}  # ts → mark-to-market equity
    ...
    def _check_exits(self, bar: Bar) -> list[Trade]:
        ...
        SL/TP are checked against intrabar HIGH/LOW and filled at the
        stop/limit price.  Max-bars exits fill at bar.close.
    ...
    def _update_portfolio_value(self, bar: Bar) -> None:
        """Record mark-to-market equity snapshot for the current bar.

        Equity = initial_capital + realised_pnl + sum(unrealised_pnl)
        where unrealised_pnl is computed for every open position at
        the current bar's close.

        Args:
            bar: The current bar (provides timestamp and close price).
        """
        ts = bar.timestamp
        if hasattr(ts, "to_pydatetime"):
            ts = ts.to_pydatetime()

        # Mark-to-market: value all open positions at current close
        unrealised = 0.0
        for ticker, trade in self.open_positions.items():
            ...
            if ticker == bar.ticker:
                ...
                # Use last known price for other tickers (stored below)
                price = self._last_prices.get(ticker, trade.entry_price)
            if trade.is_long:
        ...
        # Cache the latest close per ticker for mark-to-market
        self._last_prices[bar.ticker] = bar.close

        mtm_equity = self.capital + self._realised_pnl + unrealised
        # Overwrite same timestamp (multiple tickers per bar)
        self._equity_events[ts] = mtm_equity
